...
public class JUnitTest {
    ...
    public void test7() {
        ...
        List<? extends Animal1> l1 = new ArrayList<>();// <=src.Animal1
        List<? super Animal1> l2 = new ArrayList<>();// >=src.Animal1
        Animal1 animal = l1.get(0);// 拿 >= Animal1的类接收
        Object object = l2.get(0);// 拿最大的类接收
        // src.Animal1 p = l2.get(0); //编译不通过因为得到的类有可能比Animal1大

        // l1.add(new src.Animal1());// 编译不通过因为得到的类有可能比Animal1还小
        // l1.add(new src.Dog()); // 编译不通过因为得到的类有可能比Dog还小
        l2.add(new Animal1());
        l2.add(new Dog());
    }
